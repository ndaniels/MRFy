...
data SearchStrategy placement = 
 SS { gen0    :: Seed -> ScoredPopulation placement
    ...
    , toApproved :: [Seed] -> Age
      ...
      -- Only the best solution is taken from each placement
    }

type SearchStop a = [Aged (ScoredPopulation a)] -> (Scored a, S.History a)
-- @ end strategy.tex

data ExtendedSearchStrategy a = ESS { strategy :: SearchStrategy a
...
-- XXX TODO too many calls to 'minimum'!
